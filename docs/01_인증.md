...
- [비관리형 결제 가이드](./07-2\_Self\_결제.md)에서 Payment-Key 연동하기

## 사전 준비 작업

- API 인증키를 발급받으려면 스텝페이 포탈에서 회원가입이 필요합니다.
- 결제 연동키 발급 후 스텝페이 포탈에서 [PG 설정](./07-0\_결제.md#사전-준비-작업)이 필요합니다.
...
Payment-Key는 [비관리형 결제](./07-2\_Self\_결제.md)를 사용하는 가맹점의 경우 ‘결제 SDK’를 사용하게 되며, 이때 Payment-Key가 필요합니다.\
Payment-Key를 사용하여 결제를 연동하는 경우, [PG 설정](./07-0\_결제.md#사전-준비-작업)을 통해 사용하고자 하는 PG가 설정되어 있어야 합니다.
![인증키\_관리.png](https://dev-vercel-dev-steppaykr.vercel.app/api/localize?dir=01\_auth\&name=setting_pg.png)
